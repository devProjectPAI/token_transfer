...
export const apiSwap = async () => {
  ...
  const outputMint = 'paiiRLHDZdvZ5iqsc8Xmn9YjioCtki4wvQex2e8xRY9' // PAI

  //const inputMint = 'paiiRLHDZdvZ5iqsc8Xmn9YjioCtki4wvQex2e8xRY9' // PAI
  ...
  let tokenAmount = await connection.getTokenAccountBalance(outputTokenAcc!);
  ...
  let amount = Math.floor(sol_amount * LAMPORTS_PER_SOL);
  console.log(`Use : ${amount} SOL to buy PAI`);
  ...
  if (!isV0Tx) {
    ...
    for (const tx of allTransactions) {
      ...
      // we have no solana RPC server, so there is no ws server for confirmTransaction, just don't call it
      /*
